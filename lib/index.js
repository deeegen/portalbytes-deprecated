...
module.exports = class {
  constructor(prefix = "/web/", config = {}) {
    ...
    // Enhanced URL proxification with complete URL encoding
    this.proxifyRequestURL = (url, type) => {
      if (type) {
        // Decode the proxified URL back to original
        try {
          // Handle the new full URL encoding format: _base64(fullURL)_
          if (url.startsWith('_') && url.includes('_/')) {
          ...
          return decodedOrigin + pathPart;
        } catch (e) {
          console.error("Failed to decode URL:", e);
          return url;
        }
      } else {
        // Encode complete URL to proxified format
        try {
          // New format: encode the entire URL for better path handling
  ...
  http(req, res, next = () => res.end("")) {
    ...
    if (typeof this.config.blacklist == "object" && this.config.blacklist.length != 0) {
      this.config.blacklist.forEach((blacklisted) => {
        if (proxyURL.hostname === blacklisted || proxyURL.href === blacklisted || proxyURL.href.includes(blacklisted)) {
          isBlocked = true;
        }
      });
    }
    ...
    // Enhanced path validation - check for the new encoding format
    const expectedPrefix = `_${btoa(proxyURL.href)}_`;
    if (!req.path.startsWith(`/${expectedPrefix}`)) {
      // Try old format compatibility
      const oldFormatPrefix = `_${btoa(proxyURL.origin)}_`;
      if (!req.path.startsWith(`/${oldFormatPrefix}`)) {
    ...
    if (proxyOptions.headers["origin"]) {
      try {
        const originPath = "/" + proxyOptions.headers["origin"].split("/").splice(3).join("/");
        const proxifiedOrigin = this.proxifyRequestURL(originPath.replace(this.prefix, ""), true);
        
        if (proxifiedOrigin.startsWith("https://") || proxifiedOrigin.startsWith("http://")) {
    ...
    if (proxyOptions.headers["referer"]) {
      try {
        const refererPath = "/" + proxyOptions.headers["referer"].split("/").splice(3).join("/").replace(this.prefix, "");
        const proxifiedReferer = this.proxifyRequestURL(refererPath, true);
        
        if (proxifiedReferer.startsWith("https://") || proxifiedReferer.startsWith("http://")) {
