...
module.exports = function handleHttp(req, res, next) {
    ...
    (proxyRes) => {
      ...
      proxyRes.on("end", () => {
        ...
        Object.entries(proxyRes.headers).forEach(([hdr, val]) => {
          ...
          // fully override CSP header
          if (name === "content-security-policy") {
            proxyRes.headers[hdr] = [
          ...
          // normalize and flatten CSP arrays
          if (name === "content-security-policy" && Array.isArray(val)) {
            proxyRes.headers[hdr] = val.join("; ");
          }
  ...
  function proxifyCss(buf) {
    ...
    buf = buf.replace(/url\(\s*(['"]?)([^)"'\s]+)\1\s*\)/gi, (_, quote, u) => {
      if (/^(data:|about:|javascript:|blob:|#)/i.test(u)) {
        return `url(${quote}${u}${quote})`;
      }
      return `url(${quote}${proxifyUrl(u)}${quote})`;
    });
    ...
    buf = buf.replace(
      ...
      (_, __, quote, u) => {
        ...
        return `@import url(${quote}${proxifyUrl(u)}${quote});`;
      }
    );
