...
var proxify = {
  url: (url, type) => {
    ...
    switch (type) {
      case true:
        // Enhanced decoding for full URL encoding
        try {
          // Handle new format: _base64(fullURL)_/
          ...
          const parts = url.replace(alloy.prefix, "").split("_");
          if (parts.length >= 3) {
      ...
      default:
        ...
        // Enhanced encoding: encode the complete URL
        try {
          const encodedFullUrl = btoa(url.href);
          proxified = alloy.prefix + "_" + encodedFullUrl + "_/";
        } catch (e) {
